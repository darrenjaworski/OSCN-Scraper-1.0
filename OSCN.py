...
for year in years:
    ...
    for county in counties:
        exists = 0
        counter = 42# Put number of last page scraped here
        ...
        while exists <= 15:
            ...
            if len(root.cssselect("table")) != 8: # The method len() gives the total length of the dictionary. This would be equal to the number of items in the dictionary.Check if page is a case record page or the 404 page (record pages have 8 tables, errors have 11)
                exists += 1
                ...
                for tr in root.cssselect("table")[6]:
                    d = 0
                    tds = tr.cssselect("tr")
                    if tds[d].text_content().startswith("Defendant: "):
                        disposition.append(tds[d].text_content().replace("\r"," ").replace("\t","").replace("\n","").replace(u'\xa0',''))
                        d += 1
                    else:
                        d += 1
